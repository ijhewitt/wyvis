...
pd.h_rc = 1;                    % roughness height for channels [m]
...
[td,ad,ud] = wyvis_solve( [0:7:2*365]*24*60*60 , ad , pd , [] , oo); % solve model
...
ad.M_in = @(t) 1e-6 + max( (1.1e-3*wyvis_annual_signal(t,135*pd.td,244*pd.td,21*pd.td)-ad.Z_s/2e6) ,0); % distributed conduit input with elevation dependence
[td,ad,ud] = wyvis_solve( pd.td*[0:1:1*365] , ad , pd , ud0  ,oo );   % solve model
...
figure(4); 
    contourf( [ad.x]/1e3 , td/pd.td , [ud.u]'*pd.ty , 20 ); colorbar;           
    xlabel('Distance [km]');
    ylabel('Time [d]');
