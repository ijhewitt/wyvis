% ice sheet scale flat bed and sqrt surface elevation, driven by seasonal
% water input, coupled drainage system
...
ad.x = linspace(0,100*1e3,80)';           % x grid points [m]            
ad.Z_b = 0*ad.x.^0;                       % bed elevation [m]  
ad.Z_s = 2000*(1-ad.x/ad.x(end)).^(1/2);  % surface elevation [m]         
ad.M_in = @(t) ad.x.^0*(1e-8*t.^0);       % distributed input [m^2/s] 

% RUN FOR 1 YEAR TOWARDS STEADY STATE
oo.include_ice_velocity = 1;     % include calculation of ice velocty
% oo.include_a44 = 0;              % include longitudinal stresses
