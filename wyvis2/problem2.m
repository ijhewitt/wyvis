% arolla flow line, coupled drainage system, seasonal water input
...
pd.K = 1e-6;                        % sheet permeability [kg^(-1)m^(4-alpha)s]
disp(pd);                           % display parameters

% DIMENSIONAL GEOMETRY AND INPUTS
arolla;                              % load arolla flow line
ad.x = x;                            % x grid pts [m]
ad.Z_b = b;                          % bed elevation [m]         
ad.Z_s = s;                          % surface elevation [m]
ad.U_b = 0*ad.x;                     % sliding speed [m/d] (only the first entry should be used if computing ice speed)
ad.M_in = @(t) ad.x.^0*(1e-8*t.^0);  % distributed input to conduit
...
ad.M_in = @(t) ( 1e-8+1e-4*max(ad.x.^0*wyvis_annual_signal(t,135*pd.td,244*pd.td,21*pd.td)-(ad.Z_s-2500)*t.^0/1000,0) );  % update distributed conduit input
[td,ad,ud] = wyvis_solve( pd.td*[0:1:2*365] , ad , pd , ud0  ,oo );   % solve model
td1 = td; ud1 = ud;
